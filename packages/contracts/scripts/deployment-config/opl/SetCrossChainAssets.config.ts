...
const main = async function () {
    ...
    const assets = otherNetworks.flatMap((network) => {
        const net = config.networks[network];
        if (!net.chainId) {
            throw new Error(`Not chainId set for ${network}`);
        }

        const assets = ASSETS_LIST[chainIdToEnumChainId(net.chainId.toString())];
    ...
    await CrossChainVaultApp.setCrossChainAssets(assets);
}
